...
_prevchain: std::ptr::null_mut(),
_mode: Default::default(),
_unused2: [Default::default(); 20],
}}}
...
pub unsafe extern "C" fn csv_fini(
    ...
    match pstate {
        3 => {
            (*p.as_deref_mut().unwrap()).entry_pos= (*p.as_deref().unwrap()).entry_pos
                .wrapping_sub((*p.as_deref().unwrap()).spaces.wrapping_add(1 as libc::c_int as size_t));
            entry_pos= (*p.as_deref().unwrap()).entry_pos;
            current_block_26= 16085665132206794377;
        }
        1 | 2 => {
            current_block_26= 16085665132206794377;
        }
...
unsafe extern "C" fn csv_increase_buffer(mut p: Option<&mut csv_parser>) -> libc::c_int {
    ...
    (*p.as_deref_mut().unwrap()).entry_size= (*p.as_deref().unwrap()).entry_size.wrapping_add(to_add);
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn csv_parse(
    ...
    while pos < len {
        if entry_pos
            == (if (*p.as_deref().unwrap()).options as libc::c_int & 8 as libc::c_int != 0 {
                (*p.as_deref().unwrap()).entry_size.wrapping_sub(1 as libc::c_int as size_t)
            } else {
