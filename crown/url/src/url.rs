...
#[inline]
unsafe extern "C" fn url_get_hash(mut url: *const libc::c_char) -> *mut /* owning */ libc::c_char {
...
unsafe extern "C" fn scan_part(
    ...
    loop {
        ...
        if crate::src::url::char_cat[(*p) as libc::c_uchar as usize] as libc::c_uint
...
unsafe extern "C" fn parse_query_string(
    ...
    loop {
        ...
        if kv_end.is_null() {();
            ...
            current_block= 9504180097263930425;
            break;
        } else {
            ...
            *key_end= '\0' as i32 as libc::c_char;
            (*kv.offset(element as isize)).key = decode_percent(key);
            if has_value {
                let mut value = key_end.offset(1 as libc::c_int as isize);
                (*kv.offset(element as isize)).value = decode_percent(value);
            } else {
...
#[no_mangle]
pub unsafe extern "C" fn url_parse(mut url: *const libc::c_char) -> *mut /* owning */ url_data_t {
    ...
    if p.is_null() {();
        ...
        (*data).whole_url= p;
        p_end= p.offset(strlen(p as *const i8) as isize);
                        ...
                        if p >= p_end as *mut libc::c_char {
                            ...
                            if (*p) as libc::c_int == '[' as i32 {
                                ...
                                if hostname_end.is_null() {();
                                    ...
                                    current_block= 1949204620444267788;
                                } else {
                                    *hostname_end= '\0' as i32 as libc::c_char;
                                    (*data).host= p as *const i8;
                                        ...
                                        && (*hostname_end) as libc::c_int == ':' as i32
                                    {
                                        ...
                                        if !port_end.is_null() {
                                            (*data).port= hostname_end.offset(1 as libc::c_int as isize);
                                            p= port_end;
...
pub unsafe extern "C" fn url_is_protocol(mut str: *const libc::c_char) -> bool {
    ...
    while i < count {
        if 0 as libc::c_int == strcmp(crate::src::url::URL_SCHEMES[i as usize], str) {
...
pub unsafe extern "C" fn url_get_query_value(
    ...
    if (*url).query as *const crate::src::url::url_key_value.is_null() {();
        return 0 as *const libc::c_char;
    }
    let mut kv = (*url).query as *const crate::src::url::url_key_value;
...
pub unsafe extern "C" fn url_data_inspect(mut data: *const url_data_t) {
    ...
    if !(*data).query as *const crate::src::url::url_key_value.is_null() {
        let mut nr = 0 as libc::c_int as libc::c_uint;
        while !((*(*data).query as *const crate::src::url::url_key_value.offset(nr as isize)).key).is_null() {
            printf(
                b"    .query[%u]: \"%s\" -> \0" as *const u8 as *const libc::c_char,
                nr,
                (*(*data).query as *const crate::src::url::url_key_value.offset(nr as isize)).key,
            );
            if !((*(*data).query as *const crate::src::url::url_key_value.offset(nr as isize)).value).is_null() {
                printf(
                    b"\"%s\"\n\0" as *const u8 as *const libc::c_char,
                    (*(*data).query as *const crate::src::url::url_key_value.offset(nr as isize)).value,
                );
            } else {();
                printf(b"(NULL)\n\0" as *const u8 as *const libc::c_char);
            }
...
#[no_mangle]
pub unsafe extern "C" fn url_free(mut data: *mut /* owning */ url_data_t) {
    if data.is_null() {();
        return;
    }
