...
pub type __builtin_va_list = [crate::src::optipng::__va_list_tag; 1];
#[derive(Copy, Clone)]
...
pub type opng_ulongest_impl_t = opng_ullong_t;
...
unsafe extern "C" fn opng_sprint_uratio_impl(
    mut buffer: *mut libc::c_char,
    mut buffer_size: size_t,
    mut num: opng_ulongest_impl_t,
    mut denom: opng_ulongest_impl_t,
    mut always_percent: libc::c_int,
) -> libc::c_int {
    let mut integer_part: opng_ulongest_impl_t = 0;
    let mut remainder: opng_ulongest_impl_t = 0;
    let mut fractional_part: libc::c_uint = 0;
    let mut scale: libc::c_uint = 0;
    let mut scaled_ratio: libc::c_double = 0.;
    if denom == 0 as libc::c_int as opng_ulongest_impl_t {
        return opng_snprintf_impl(
            buffer,
            buffer_size,
            if num == 0 as libc::c_int as opng_ulongest_impl_t {
                b"??%%\0" as *const u8 as *const libc::c_char
            } else {
                b"INFTY%%\0" as *const u8 as *const libc::c_char
            },
        );
    }
    if num < denom
        && denom / denom.wrapping_sub(num) < 20000 as libc::c_int as opng_ulongest_impl_t
    {
    ...
    if integer_part < 100 as libc::c_int as opng_ulongest_impl_t {
        return opng_snprintf_impl(
            buffer,
            buffer_size,
    ...
    integer_part = num / denom;
    if remainder
        > denom.wrapping_sub(1 as libc::c_int as opng_ulongest_impl_t)
            / 2 as libc::c_int as opng_ulongest_impl_t
    {
        integer_part = integer_part.wrapping_add(1);
        integer_part;
    }
...
pub unsafe extern "C" fn opng_ulratio_to_factor_string(
    mut buffer: *mut libc::c_char,
    mut buffer_size: size_t,
    mut ratio: *const crate::src::optim::opng_ulratio,
) -> libc::c_int {
    let mut num = (*ratio).num as opng_ulongest_impl_t;
    let mut denom = (*ratio).denom as opng_ulongest_impl_t;
...
pub unsafe extern "C" fn opng_ullratio_to_factor_string(
    ...
    let mut num = (*ratio).num;
    let mut denom = (*ratio).denom;
    return opng_sprint_uratio_impl(buffer, buffer_size, num, denom, 0 as libc::c_int);
}
#[no_mangle]
