...
pub type png_struct = crate::src::optim::png_struct_def;
...
static mut options: crate::src::optim::opng_options = crate::src::optim::opng_options {
    backup: 0,
    clobber: 0,
    debug: 0,
    fix: 0,
        ...
        ch1 = ({
                ...
                > 1 as libc::c_int as libc::c_ulong
            {
                if 0 != 0 {
                    let fresh0 = str1;
                    str1 = str1.offset(1);
                    let mut __c = *fresh0 as libc::c_int;
        ...
        ch2 = ({
                ...
                > 1 as libc::c_int as libc::c_ulong
            {
                if 0 != 0 {
                    let fresh3 = str2;
                    str2 = str2.offset(1);
                    let mut __c = *fresh3 as libc::c_int;
                    __res = if __c < -(128 as libc::c_int) || __c > 255 as libc::c_int {
                        __c
                    } else {
                        *(*__ctype_tolower_loc()).offset(__c as isize)
...
unsafe extern "C" fn opng_str2ulong(
    ...
    let mut begin_ptr = 0 as *const libc::c_char;
    let mut end_ptr = 0 as *mut libc::c_char;
    ...
    {
        *out_val = strtoul(begin_ptr, &raw mut end_ptr, 10 as libc::c_int);
    }
    ...
    if *opng_strltrim(end_ptr) as libc::c_int != 0 as libc::c_int {
        *__errno_location() = 22 as libc::c_int;
        return -(1 as libc::c_int);
    }
    return 0 as libc::c_int;
}
unsafe extern "C" fn opng_rangeset2bitset(
    mut out_val: *mut opng_bitset_t,
    mut in_str: *const libc::c_char,
) -> libc::c_int {
    let mut end_idx: size_t = 0;
    *out_val = crate::src::bitset::opng_rangeset_string_to_bitset(in_str, &raw mut end_idx);
    if end_idx == 0 as libc::c_int as size_t
        || *opng_strltrim(in_str.offset(end_idx as isize)) as libc::c_int
            != 0 as libc::c_int
    {
        *__errno_location() = 22 as libc::c_int;
        return -(1 as libc::c_int);
...
unsafe extern "C" fn err_option_arg(
    ...
    if opt_arg.is_null() || *opng_strltrim(opt_arg) as libc::c_int == 0 as libc::c_int {
        error(
...
unsafe extern "C" fn check_num_option(
    ...
    if opng_str2ulong(&raw mut value, opt_arg, 0 as libc::c_int) != 0 as libc::c_int
...
unsafe extern "C" fn check_power2_option(
    ...
    if opng_str2ulong(&raw mut value, opt_arg, 1 as libc::c_int) == 0 as libc::c_int {
...
unsafe extern "C" fn check_rangeset_option(
    ...
    if opng_rangeset2bitset(&raw mut result, opt_arg) == 0 as libc::c_int {
        result &= result_mask;
    } else {
        result = 0 as libc::c_int as opng_bitset_t;
    }
    if result == 0 as libc::c_int as opng_bitset_t {
        err_option_arg(opt, opt_arg);
    }
    return result;
}
unsafe extern "C" fn check_obj_option(
    ...
    if i == 4 as libc::c_int as libc::c_uint
        && *opt_arg.offset(i as isize) as libc::c_int == 0 as libc::c_int
    {
        error(
...
unsafe extern "C" fn scan_option(
    ...
    loop {
                ...
                & _ISspace as libc::c_int as libc::c_ushort as libc::c_int != 0
        {
            ...
            *opt_arg_ptr = if *ptr as libc::c_int != 0 as libc::c_int {
                ptr
            } else {
...
unsafe extern "C" fn parse_args(
    ...
    let mut xopt = 0 as *const libc::c_char;
    ...
    memset(
        &raw mut options as *mut crate::src::optim::opng_options as *mut libc::c_void,
        0 as libc::c_int,
        ::core::mem::size_of::<crate::src::optim::opng_options>() as libc::c_ulong,
    );
    ...
    while i < argc {
            ...
            ) < 1 as libc::c_int
        {
            ...
            *argv.offset(i as isize) = 0 as *mut libc::c_char;
            if !(strchr(
            ...
            ) == 0 as libc::c_int && opt_len >= 4 as libc::c_int as size_t
            {
                ...
                if xopt.is_null() {std::intrinsics::assume((xopt).addr() == 0);
                    i += 1;
                    if i < argc {
            ...
            if simple_opt != 0 {
                ...
                }else { std::intrinsics::assume((xopt).addr() == 0); }
            } else if strcmp(
            ...
            ) == 0 as libc::c_int && opt_len >= 2 as libc::c_int as size_t
            {
                ...
                if *xopt.offset(0 as libc::c_int as isize) as libc::c_int
                    == 0 as libc::c_int
                {
                    err_option_arg(
                        b"-out\0" as *const u8 as *const libc::c_char,
            ...
            ) == 0 as libc::c_int
            {
                ...
                }else { std::intrinsics::assume((options.log_name).addr() == 0); }
                if *xopt.offset(0 as libc::c_int as isize) as libc::c_int
                    == 0 as libc::c_int
                {
                    err_option_arg(
...
unsafe extern "C" fn app_printf(mut fmt: *const libc::c_char, mut args: ...) {
    let mut arg_ptr: ::core::ffi::VaListImpl;
    if *fmt.offset(0 as libc::c_int as isize) as libc::c_int == 0 as libc::c_int {
        return;
    }
    ...
    }else { std::intrinsics::assume((con_file).addr() == 0); }
    if !log_file.is_null() {
        arg_ptr = args.clone();
        vfprintf(log_file, fmt, arg_ptr.as_va_list());
    }else { std::intrinsics::assume((log_file).addr() == 0); }
}
unsafe extern "C" fn app_print_cntrl(mut cntrl_code: libc::c_int) {
    let mut con_str = 0 as *const libc::c_char;
    let mut log_str = 0 as *const libc::c_char;
    let mut i: libc::c_int = 0;
...
unsafe extern "C" fn app_init() {
    ...
    if !(options.log_name).is_null() {
        ...
        if log_file.is_null() {std::intrinsics::assume((log_file).addr() == 0);
            error(
...
unsafe extern "C" fn process_files(
    ...
    let mut result: libc::c_int = 0;
    let mut ui = crate::src::optim::opng_ui {
        printf_fn: None,
        print_cntrl_fn: None,
        progress_fn: None,
        panic_fn: None,
    ...
    if crate::src::optim::opng_initialize(&raw mut options, &raw mut ui) != 0 as libc::c_int {
        panic(
    ...
    while i < argc {
        ...
        {
            if crate::src::optim::opng_optimize(*argv.offset(i as isize)) != 0 as libc::c_int {
    ...
    if crate::src::optim::opng_finalize() != 0 as libc::c_int {
        panic(
